...
from fastapi import APIRouter, BackgroundTasks, Depends, Query, Request, status, UploadFile, File
...
@router.post("/api/v1/admin/usuarios-empresa/{id_matricula}/reset-password", tags=["Admin"])
...
async def api_upload_csv(
    ...
    id_evento: int = Query(..., description="ID del evento al que se asignarán los proyectos"),
    db: AsyncSession = Depends(get_db),
    _=Depends(get_current_admin),
):
    """
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail="No se pudo decodificar el archivo. Usa codificación UTF-8 o Latin-1")
